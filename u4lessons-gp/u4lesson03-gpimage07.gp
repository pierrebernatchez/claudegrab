...
set output '../u4lessons-media/u4lesson03-gpimage07.png'
...
set print
set arrow from -3*pi/2,-5 to -3*pi/2,5 nohead dashtype 2 lc rgb 'green' lw 1.2
set arrow from -pi/2,-5 to -pi/2,5 nohead dashtype 2 lc rgb 'green' lw 1.2
set arrow from pi/2,-5 to pi/2,5 nohead dashtype 2 lc rgb 'green' lw 1.2
set arrow from 3*pi/2,-5 to 3*pi/2,5 nohead dashtype 2 lc rgb 'green' lw 1.2
h(x) = (abs(cos(x)) < 0.03) ? (1/0) : 1.0/cos(x)
plot cos(x) with lines lc rgb 'red' lw 1.2, \
     h(x) with lines lc rgb 'blue' lw 1.3, \
