3.4 Résolution des équations exponentielles -- Feuille de travail
################################################################################

:lang: fr
:date: 2026-09-12 17:00:00+00:00
:tags: 12e année, mathématiques, leçons, feuilles de travail
:slug: u3worksheet04-fr
:category: mathematics
:authors: Annie Bernatchez
:summary: 3.4 Résolution des équations exponentielles -- Feuille de travail
:fcopyright: Copyright © 2026 Annie Bernatchez—Tous droits réservés.

.. |copy| unicode:: 0xA9

.. |---| unicode:: U+02014
  :trim:

.. |nbsp| unicode:: 0xA0

.. footer:: Copyright |copy| 2026 Annie Bernatchez |---| Tous droits réservés.

Ce document a été composé et mis en forme par Annie Bernatchez.

Le contenu du cours provient de `www.jensenmath.ca <https://www.jensenmath.ca/>`_

.. rst-class:: keepwithnext

**1)** Écris chaque expression avec la base 2.

.. rst-class:: keepwithnext

a\) :math:`4^6`

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

b\) :math:`8^3`

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

c\) :math:`\left(\dfrac{1}{8}\right)^2`

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

d\) :math:`14`

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

**2)** Écris chaque expression comme une puissance de 4.

.. rst-class:: keepwithnext

a\) :math:`\left(\sqrt{16}\right)^3`

``_____``

.. rst-class:: keepwithnext

b\) :math:`\sqrt[3]{16}`

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

c\) :math:`\sqrt{64} \times \left(\sqrt[4]{128}\right)^3`

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

**3)** Résous chaque équation

.. rst-class:: keepwithnext

a\) :math:`2^{4x} = 4^{x+3}`

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

b\) :math:`3^{w+1} = 9^{w-1}`

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

c\) :math:`4^{3x} = 8^{x-3}`

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

d\) :math:`125^{2y-1} = 25^{y+4}`

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

**4)** Considère l'équation :math:`10^{2x} = 100^{2x-5}`

.. rst-class:: keepwithnext

a\) Résous cette équation en exprimant les deux côtés comme des
puissances d'une base commune.

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

b\) Résous la même équation en prenant le logarithme commun des deux
côtés.

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

**5)** Résous :math:`2^{3x} > 4^{x+1}`

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

**6)** Résous pour :math:`t`. Arrondis tes réponses à 2 décimales.

.. rst-class:: keepwithnext

a\) :math:`2 = 1.07^t`

|nbsp|

.. rst-class:: keepwithnext

b\) :math:`100 = 10(1.04)^t`

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

c\) :math:`15 = \left(\dfrac{1}{2}\right)^{\frac{t}{4}}`

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

**7)** Résous chaque équation. Arrondis tes réponses à 3 décimales.

.. rst-class:: keepwithnext

a\) :math:`2^x = 3^{x-1}`

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

b\) :math:`5^{x-2} = 4^x`

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

c\) :math:`7^{2x+1} = 4^{x-2}`

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

**8)** Résous :math:`2^{2x} + 2^x - 6 = 0` à l'aide de la formule
quadratique (ou par factorisation). Identifie clairement toute racine
étrangère.

|nbsp|

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

**9)** Résous :math:`8^{2x} - 2(8^x) - 5 = 0` à l'aide de la formule
quadratique. Identifie clairement toute racine étrangère.

|nbsp|

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

**10)** Utilise l'équation de décroissance du polonium-218, :math:`A(t) =
A_0 \left(\dfrac{1}{2}\right)^{\frac{t}{3.1}}`, où :math:`A` est la
quantité restante après :math:`t` minutes et :math:`A_0` est la quantité
initiale.

.. rst-class:: keepwithnext

a\) Combien restera-t-il après 90 secondes, à partir d'un échantillon
initial de 50 mg?

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

b\) Combien de temps faudra-t-il pour que cet échantillon se dégrade
jusqu'à 10 % de sa quantité initiale de 50 mg?

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

**11)** Un échantillon de 20 mg de thorium-233 se dégrade à 17 mg après 5
minutes.

.. rst-class:: keepwithnext

a\) Quelle est la demi-vie du thorium-233?

|nbsp|

|nbsp|

|nbsp|

.. rst-class:: keepwithnext

b\) Combien de temps faudra-t-il pour que cet échantillon se dégrade
jusqu'à 1 mg?

|nbsp|

|nbsp|

|nbsp|

Corrigé
================================================================================

**1)** a) :math:`2^{12}` b) :math:`2^9` c) :math:`2^{-6}` d)
:math:`2^{\frac{\log 14}{\log 2}}`

**2)** a) :math:`4^3` b) :math:`4^{\frac23}` c) :math:`4^{\frac{33}{8}}`

**3)** a) 3 b) 3 c) -3 d) :math:`\dfrac{11}{4}`

**4)** a) 5 b) 5

**5)** :math:`x > 2`

**6)** a) 10.24 b) 58.71 c) -15.63

**7)** a) 2.710 b) 14.425 c) -1.883

**8)** :math:`x = 1` est la seule solution; :math:`2^x = -3` ou
:math:`x = \dfrac{\log(-3)}{\log 2}` est une racine étrangère

**9)** :math:`x = \dfrac{\log(1+\sqrt{6})}{\log 8} \cong 0.6` est la
seule solution; :math:`8^x = 1 - \sqrt{6}` ou :math:`x =
\dfrac{\log(1-\sqrt{6})}{\log 8}` est une racine étrangère

**10)** a) 35.75 mg b) 10.3 min

**11)** a) 21.3 min b) 92.06 min
