1.5 Leçon sur la symétrie des fonctions polynomiales
################################################################################

:lang: fr
:date: 2026-09-07 02:20:16+00:00
:tags: douzième année, mathématiques, leçons, fiches d’exercices
:slug: lesson05
:category: mathématiques
:authors: Annie Bernatchez
:summary: 1.5 Leçon sur la symétrie des fonctions polynomiales
:fcopyright: Droits d´auteur © 2026 Annie Bernatchez—Tous droits reservés.

.. |copy| unicode:: 0xA9

.. |---| unicode:: U+02014
  :trim:

.. |nbsp| unicode:: 0xA0

.. footer:: Droits d´auteur |copy| 2026 Annie Bernatchez |---| Tous droits reservés.

Ce document a été composé et mis en forme par Annie Bernatchez.

Le contenu du cours provient de `www.jensenmath.ca <https://www.jensenmath.ca/>`_

Dans cette section, vous apprendrez les propriétés des fonctions
polynomiales paires et impaires.

Symétrie des fonctions polynomiales
================================================================================

``__________________`` -- il existe une droite verticale par rapport à
laquelle le polynôme reste inchangé lorsqu'il est réfléchi.

``___________________________________`` -- il existe un point autour
duquel le polynôme reste inchangé lorsqu'il est tourné de 180°

Section 1 : Propriétés des fonctions paires et impaires
================================================================================

Une fonction polynomiale de degré pair ou impair n'est PAS
nécessairement une fonction paire ou impaire. Voici les propriétés de
toutes les fonctions paires et impaires :

.. list-table::
   :widths: 50 50
   :header-rows: 0

   * - **Fonctions paires**
     - **Fonctions impaires**
   * - Une fonction polynomiale de degré pair est une FONCTION PAIRE
       si :

       - Symétrie par rapport à ``__________``
       - L'exposant de chaque terme est ``_______``
       - Peut avoir un terme constant
     - Une fonction polynomiale de degré impair est une FONCTION
       IMPAIRE si :

       - Symétrie par rapport à ``__________``
       - L'exposant de chaque terme est ``_____``
       - Aucun terme constant
   * - **Règle :**

       |nbsp|
     - **Règle :**

       |nbsp|
   * - **Exemple :**

       .. image:: ../images/lesson05-image03.png
          :scale: 50
          :alt: f(x) = 2x^4 + 3x^2 - 2 with f(1) and f(-1) marked

       :math:`f(x) = 2x^4 + 3x^2 - 2`

       Remarquez :

       :math:`f(1) =` ``____``

       :math:`f(-1) =` ``____``

       :math:`\therefore` ``________________________``
     - **Exemple :**

       .. image:: ../images/lesson05-image04.png
          :scale: 50
          :alt: f(x) = 2x^3 + 3x with f(1) and f(-1) marked

       :math:`f(x) = 2x^3 + 3x`

       Remarquez :

       :math:`f(1) =` ``____``

       :math:`f(-1) =` ``____``

       :math:`\therefore` ``________________________``

**Exemple 1 :** Déterminez si chaque fonction est une fonction paire,
impaire, ou ni l'une ni l'autre. Expliquez comment vous pouvez le
savoir.

a\) :math:`y = x^3 - 4x`

.. image:: ../images/lesson05-image05.png
   :scale: 50
   :alt: y = x^3 - 4x, point (0,0) marked

b\) :math:`y = x^3 - 4x + 2`

.. image:: ../images/lesson05-image06.png
   :scale: 50
   :alt: y = x^3 - 4x + 2, point (0,2) marked

c\) :math:`y = x^4 - 4x^2 + 2`

.. image:: ../images/lesson05-image07.png
   :scale: 50
   :alt: y = x^4 - 4x^2 + 2

d\) :math:`y = 3x^4 + x^3 - 4x^2 + 2`

.. image:: ../images/lesson05-image08.png
   :scale: 50
   :alt: y = 3x^4 + x^3 - 4x^2 + 2

e\) :math:`y = -3x^2 - 6x`

.. image:: ../images/lesson05-image09.png
   :scale: 50
   :alt: y = -3x^2 - 6x

**Exemple 2 :** Cochez toutes les réponses qui s'appliquent à chaque
fonction

a\)

.. image:: ../images/lesson05-image10.png
   :scale: 50
   :alt: graph for Example 2a

i\) aucune symétrie

ii\) symétrie par rapport à un point

iii\) symétrie par rapport à une droite

iv\) fonction impaire

v\) fonction paire

b\)

.. image:: ../images/lesson05-image11.png
   :scale: 50
   :alt: graph for Example 2b

i\) aucune symétrie

ii\) symétrie par rapport à un point

iii\) symétrie par rapport à une droite

iv\) fonction impaire

v\) fonction paire

c\) :math:`P(x) = 5x^3 + 3x^2 + 2`

i\) aucune symétrie

ii\) symétrie par rapport à un point

iii\) symétrie par rapport à une droite

iv\) fonction impaire

v\) fonction paire

*Remarque :*

d\) :math:`P(x) = x^6 + x^2 - 11`

i\) aucune symétrie

ii\) symétrie par rapport à un point

iii\) symétrie par rapport à une droite

iv\) fonction impaire

v\) fonction paire

e\)

.. image:: ../images/lesson05-image12.png
   :scale: 50
   :alt: graph for Example 2e

i\) aucune symétrie

ii\) symétrie par rapport à un point

iii\) symétrie par rapport à une droite

iv\) fonction impaire

v\) fonction paire

f\)

.. image:: ../images/lesson05-image13.png
   :scale: 50
   :alt: graph for Example 2f

i\) aucune symétrie

ii\) symétrie par rapport à un point

iii\) symétrie par rapport à une droite

iv\) fonction impaire

v\) fonction paire

g\) :math:`P(x) = 5x^5 - 4x^3 + 8x`

i\) aucune symétrie

ii\) symétrie par rapport à un point

iii\) symétrie par rapport à une droite

iv\) fonction impaire

v\) fonction paire

**Exemple 3 :** Sans tracer le graphique, déterminez si chaque fonction
polynomiale a une symétrie par rapport à l'axe des :math:`y`, une
symétrie par rapport à l'origine, ou ni l'une ni l'autre. Vérifiez
votre réponse algébriquement.

a\) :math:`f(x) = 2x^4 - 5x^2 + 4`

b\) :math:`f(x) = -3x^5 + 9x^3 + 2x`

c\) :math:`x^6 - 4x^3 + 6x^2 - 4`

Section 2 : Liens avec l'ensemble de l'unité
================================================================================

**Exemple 4 :** Utilisez le graphique donné pour indiquer :

.. image:: ../images/lesson05-image14.png
   :scale: 50
   :alt: quintic with local max (-1, 3), touching at -2 and 2, crossing at 0

a\) les zéros

b\) le nombre de points tournants

c\) le degré minimal possible

b\) toute symétrie présente

c\) les intervalles où :math:`f(x) < 0`

d\) Déterminez l'équation sous forme factorisée
