...
- A polynomial function of degree :math:`n` has at most
  ``____________`` local max/min points (turning points)
- A polynomial function of degree :math:`n` may have up to ``____``
  distinct zeros (x-intercepts)
- If a polynomial function is ``________`` degree, it must have at least
