...
b\) What is the sign of the leading coefficient?

c\) What are the :math:`x`-intercepts?

d\) What is the :math:`y`-intercept?
...
function :math:`f(x) = (x-2)(x+1)` has :math:`x`-intercepts at ``____`` and
``____``. These are the roots of the equation :math:`(x-2)(x+1) = 0`.

If a polynomial function has a factor :math:`(x-a)` that is
repeated :math:`n` times, then :math:`x = a` is a zero of ``_________`` :math:`n`.
...
``______`` at :math:`x = 3` and a zero of order ``______`` at :math:`x = 1`.

The graph of a polynomial function changes sign at zeros of ``_________``
order but does not change sign at zeros of ``_________`` order.
