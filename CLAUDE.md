...
Course content is organized one level deeper than the repo root, under a
`unitN/` directory per unit (e.g. `unit1/`, `unit2/`, ...) — this exists
because the naming convention below is only unique *within* a unit
(unit2's `lesson01.pdf` would otherwise collide with unit1's). Everything
described below lives inside each `unitN/` directory:

- `unitN/lessons/` — the `.rst` source documents for both lessons and
  worksheets (student version with fill-in blanks, and the solutions
  version), rendered externally (e.g. to PDF) outside this repo.
- `unitN/lessons-gp/` — the `gnuplot` scripts, one per figure.
- `unitN/images/` — the `.png` files each `gnuplot` script emits. Sibling
  to both `unitN/lessons/` and `unitN/lessons-gp/`, so the relative path
  `../images/<file>.png` resolves identically from either directory —
  this is intentional, so the same path never needs to change between
  the `.rst`'s `image::` directive and the `.gp` script's `set output`
  line.
- `unitN/lessons-pdfs/` — rendered PDFs of the `.rst` documents, dropped
  in for review/evaluation. Not generated by anything in this repo.
- Original source PDFs being transcribed (e.g. `unitN/lesson1.pdf`) and
  their per-lesson instructions files (e.g.
  `unitN/lesson1-instructions.txt`) live directly in `unitN/`, since
  they're reference input rather than build output.
...
As of 2026-09-07 only `unit1/` exists (5 lessons, complete). When
starting a new unit, create `unitN/` fresh and mirror this same
`lessons/`/`lessons-gp/`/`images/`/`lessons-pdfs/` structure inside it —
don't assume a shared/top-level `lessons-gp/` etc. across units.
...
- `unitN/lessons/lessonNN-en.rst` / `unitN/lessons/worksheetNN-en.rst` —
  student version, blanks preserved.
- `unitN/lessons/lessonNN-solutions-en.rst` /
  `unitN/lessons/worksheetNN-solutions-en.rst` — solutions version, blanks
  filled in.
- `unitN/lessons-gp/lessonNN-imageMM.gp` /
  `unitN/lessons-gp/worksheetNN-imageMM.gp` — one script per figure, `MM`
  ...
  `MM` sequence at `01`).
- `unitN/images/lessonNN-imageMM.png` /
  `unitN/images/worksheetNN-imageMM.png` — the corresponding rendered
  figure. A `.gp` script's `set output` must read
  `'../images/lessonNN-imageMM.png'` (or the `worksheetNN` equivalent),
  and it must be invoked with `unitN/lessons-gp/` as the working
  directory:
  ```
  cd unitN/lessons-gp && gnuplot lessonNN-imageMM.gp
  ```
- `unitN/lessons-pdfs/lessonNN-en.pdf` /
  `unitN/lessons-pdfs/lessonNN-solutions-en.pdf` (and the `worksheetNN`
...
- **Lettered/roman labels like `a)`, `b)`, `i)`, `ii)` get silently
  ...
  ```
  python3 -c "from docutils.core import publish_doctree; publish_doctree(open('unitN/lessons/lessonNN-en.rst').read())"
  ```
...
- **A line starting at column 0 with `:math:` (or any other inline role,
  ...
  it — check with `grep -rn '^:math:' unitN/lessons/*.rst` (should return
